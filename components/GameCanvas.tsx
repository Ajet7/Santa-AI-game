...
interface GameCanvasProps {
  gameState: GameState;
  // FIX: Update onGameOver to expect a reason, making the component's output clearer.
  onGameOver: (reason: 'collision') => void; // Called on collision
...
const SantaCharacter = React.memo(({ frameCount, isJumping, totalFilesCollected, gameState }: {
  frameCount: number;
  isJumping: boolean;
  totalFilesCollected: number;
  gameState: GameState;
  ...
  const foldersForVisuals = Math.min(Math.floor(totalFilesCollected / FOLDER_CAPACITY), SANTA_BAG_CAPACITY);
  ...
  } else if (isJumping && gameState === GameState.Playing) { 
    leg1Angle = -15; 
    leg2Angle = 10;
  ...
  return (
    <div style={{ width: PLAYER_WIDTH, height: PLAYER_HEIGHT, position: 'relative', transform: 'scaleX(-1)' }} role="img" aria-label="Santa Claus character (facing left)">
      ...
      <div style={{
        ...
        display: 'flex',
        flexWrap: 'wrap',
        padding: BAG_FOLDER_ICON_SPACING,
        gap: BAG_FOLDER_ICON_SPACING,
...
const BuildingObstacle = React.memo(({ obstacle }: { obstacle: Obstacle }) => {
...
const CollectibleItem = React.memo(({ powerUp }: { powerUp: PowerUp }) => {
  if (powerUp.collected || !powerUp.collectibleFileType) return null;

  const fileTypeDetails = COLLECTIBLE_FILE_TYPES.find(ft => ft.id === powerUp.collectibleFileType);
  ...
  const tabWidth = powerUp.width * GENERIC_COLLECTIBLE_ITEM_TAB_WIDTH_FACTOR;
  ...
  return (
    <div style={style} aria-hidden="true" title={`${fileTypeDetails.label} Collectible`}>
      <div style={tabStyle} />
...
export default function GameCanvas({
  ...
  const gameLogicRef = useRef<() => void>();
  ...
  const handleJump = useCallback(() => {
    if (gameState === GameState.Playing) {
        setPlayer((prev) => {
            if (!prev.isJumping && prev.y >= GROUND_SURFACE_Y - PLAYER_HEIGHT - 5) { 
  ...
  useEffect(() => {
    gameLogicRef.current = () => {
      ...
      setObstacles((prev) =>
        prev.map((o) => ({ ...o, x: o.x - currentActualSpeed })).filter((o) => o.x + (o.width || 0) > 0)
      );

      setPowerUps((prev) =>
        prev.map((pu) => ({ ...pu, x: pu.x - currentActualSpeed })).filter((pu) => pu.x + pu.width > 0 && !pu.collected)
      );
    
      if (frameCount >= nextObstacleFrame) {
        const buildingWidth = Math.floor(Math.random() * (BUILDING_MAX_WIDTH - BUILDING_MIN_WIDTH + 1)) + BUILDING_MIN_WIDTH;
        const newObstacle: Obstacle = { id: Date.now(), x: GAME_WIDTH, width: buildingWidth, height: BUILDING_HEIGHT };
      ...
      if (frameCount >= nextCollectibleItemFrame) { 
        ...
        const newPowerUp: PowerUp = {
            id: Date.now() + 1, 
            x: GAME_WIDTH, y: fixedCollectibleYRef.current, 
            width: GENERIC_COLLECTIBLE_ITEM_WIDTH, height: GENERIC_COLLECTIBLE_ITEM_HEIGHT,
            type: 'speed_boost', collected: false, collectibleFileType: randomFileType.id,
        };
      ...
      const playerRect = { 
        x: PLAYER_INITIAL_X + PLAYER_WIDTH * 0.1, y: player.y + PLAYER_HEIGHT * 0.05,
        width: PLAYER_WIDTH * 0.65, height: PLAYER_HEIGHT * 0.9,
      };

      for (const obstacle of obstacles) {
        const obstacleRect = {
          x: obstacle.x, y: GROUND_SURFACE_Y - (obstacle.height || BUILDING_HEIGHT),
          width: obstacle.width || BUILDING_MIN_WIDTH, height: obstacle.height || BUILDING_HEIGHT,
        };
        if (
          playerRect.x < obstacleRect.x + obstacleRect.width && playerRect.x + playerRect.width > obstacleRect.x &&
          playerRect.y < obstacleRect.y + obstacleRect.height && playerRect.y + playerRect.height > obstacleRect.y
        ) {
           // FIX: Pass the 'collision' reason as required by the updated onGameOver prop signature.
           onGameOver('collision'); 
           return;
        }
      }

      setPowerUps(prevPUs => prevPUs.map(pu => {
          if (pu.collected) return pu;
          const powerUpRect = { x: pu.x, y: pu.y, width: pu.width, height: pu.height };
          if (
              playerRect.x < powerUpRect.x + powerUpRect.width && playerRect.x + playerRect.width > powerUpRect.x &&
              playerRect.y < powerUpRect.y + powerUpRect.height && playerRect.y + playerRect.height > powerUpRect.y
          ) {
      ...
      if (frameCount >= nextShootingStarFrame && shootingStars.length < MAX_SHOOTING_STARS) {
        const newShootingStar: ShootingStar = {
          id: Date.now(), x: Math.random() * GAME_WIDTH, y: -20, 
  ...
  return (
    <div
      ...
      onTouchStart={gameState === GameState.Playing ? (e) => { e.preventDefault(); handleJump(); } : undefined}
      role="application"
    ...
    >
      {staticStars.map((star, index) => (
        <div key={`star-${index}`} style={{
          ...
          backgroundColor: star.color, borderRadius: '50%',
      ...
      {shootingStars.map(ss => (
        <div key={ss.id} style={{
          ...
          width: '2px', height: ss.length,
          backgroundColor: SHOOTING_STAR_COLOR, opacity: ss.opacity,
          transform: `rotate(${ss.angle - 90}deg)`, transformOrigin: 'top left',
          zIndex: 0, borderRadius: '1px', boxShadow: `0 0 5px ${SHOOTING_STAR_COLOR}`,
        }} />
      ))}
      
      <div style={{
        position: 'absolute', left: 0, bottom: 0,
        width: '100%', height: GROUND_LEVEL_THICKNESS,
        zIndex: 1, backgroundColor: GRASS_COLOR,
      }} />
      ...
      {obstacles.map(o => <BuildingObstacle key={o.id} obstacle={o} />)}

      {powerUps.map(pu => <CollectibleItem key={pu.id} powerUp={pu} />)}
